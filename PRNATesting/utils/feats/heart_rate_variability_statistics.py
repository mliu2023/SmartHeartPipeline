...
# Local imports
from utils.feats.utils.tools import *
...
class HeartRateVariabilityStatistics:
    ...
    def calculate_heart_rate_statistics(self, heart_rate, suffix=''):
        ...
        if len(heart_rate) > 0:
            ...
            heart_rate_statistics['heart_rate_min'] = np.min(heart_rate) # keep
            heart_rate_statistics['heart_rate_max'] = np.max(heart_rate) # keep
            heart_rate_statistics['heart_rate_mean'] = np.mean(heart_rate) # keep
            heart_rate_statistics['heart_rate_median'] = np.median(heart_rate) # keep
            ...
            heart_rate_statistics['heart_rate_min'] = np.nan # keep
            heart_rate_statistics['heart_rate_max'] = np.nan # keep
            heart_rate_statistics['heart_rate_mean'] = np.nan # keep
            heart_rate_statistics['heart_rate_median'] = np.nan # keep
    ...
    def calculate_rri_temporal_statistics(self, rri, diff_rri, diff2_rri, suffix=''):
        ...
        if len(rri) > 0:
            rri_temporal_statistics['rri_min' + suffix] = np.min(rri) # keep
        #     rri_temporal_statistics['rri_max' + suffix] = np.max(rri)
        #     rri_temporal_statistics['rri_mean' + suffix] = np.mean(rri)
            rri_temporal_statistics['rri_median' + suffix] = np.median(rri) # keep
        ...
        #     rri_temporal_statistics['rri_rms' + suffix] = np.sqrt(np.mean(np.power(rri, 2)))
            ...
            rri_temporal_statistics['rri_min' + suffix] = np.nan # keep
        #     rri_temporal_statistics['rri_max' + suffix] = np.nan
        #     rri_temporal_statistics['rri_mean' + suffix] = np.nan
            rri_temporal_statistics['rri_median' + suffix] = np.nan # keep
        ...
        if len(diff_rri) > 0:
            ...
            rri_temporal_statistics['diff_rri_median' + suffix] = np.median(diff_rri) # keep
            ...
            rri_temporal_statistics['diff_rri_rms' + suffix] = np.sqrt(np.mean(np.power(diff_rri, 2))) # keep
        else:
            rri_temporal_statistics['diff_rri_min' + suffix] = np.nan
            rri_temporal_statistics['diff_rri_max' + suffix] = np.nan
            rri_temporal_statistics['diff_rri_mean' + suffix] = np.nan # keep
            rri_temporal_statistics['diff_rri_median' + suffix] = np.nan # keep
        ...
        if len(diff_rri) > 0:
            ...
            rri_temporal_statistics['pnn60' + suffix] = self.pnn(diff_rri, 0.06) # keep
            ...
            rri_temporal_statistics['pnn60' + suffix] = np.nan # keep
    ...
    def calculate_rri_cluster_statistics(self):
        ...
        if len(self.rri) > 6:
            ...
            rri_cluster_statistics['rri_cluster_ssd_2'] = ssd[1] # keep
            rri_cluster_statistics['rri_cluster_ssd_3'] = ssd[2] # keep

        else:
            # rri_cluster_statistics['rri_cluster_ssd_slope'] = np.nan
            # rri_cluster_statistics['rri_cluster_ssd_1'] = np.nan
            rri_cluster_statistics['rri_cluster_ssd_2'] = np.nan # keep
            rri_cluster_statistics['rri_cluster_ssd_3'] = np.nan # keep
