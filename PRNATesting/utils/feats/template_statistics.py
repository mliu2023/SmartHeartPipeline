    ...
    def r_peak_check(self, correlation_threshold=0.9):
        ...
        for template_id in range(self.templates.shape[1]):
            ...
            if correlation_coefficient[0, 1] < correlation_threshold:
                ...
                # Correct rpeak
                rpeak_corrected = self.rpeaks[template_id] - (np.argmax(cross_correlation) - len(self.median_template[self.template_rpeak_sp - 25:self.template_rpeak_sp + 25]))
...
#             p_wave_statistics['p_wave_eng'] = np.nan
        ...
        p_wave_statistics['p_wave_approximate_entropy_median'] = np.median(approximate_entropy) # keep
...
#         t_wave_statistics['t_wave_permutation_entropy_median'] = np.median(permutation_entropy)
        ...
        t_wave_statistics['t_wave_multiscale_permutation_entropy_median'] = np.median(multiscale_permutation_entropy) # keep
        t_wave_statistics['t_wave_multiscale_permutation_entropy_std'] = np.std(multiscale_permutation_entropy, ddof=1) # keep
...
#             pqrst_wave_statistics['pr_time_std'] = np.std(pri, ddof=1)
            # if self.templates_good.shape[1] > 1:
#                 pqrst_wave_statistics['pri_approximate_entropy'] = self.safe_check(pyeeg.ap_entropy(pri, M=2, R=0.1*np.std(pri)))
                # try:
...
#             pqrst_wave_statistics['qs_time_std'] = np.std(qsi, ddof=1)
            # if self.templates_good.shape[1] > 1:
#                 pqrst_wave_statistics['qsi_approximate_entropy'] = self.safe_check(pyeeg.ap_entropy(pri, M=2, R=0.1*np.std(qsi)))
                # try:
...
#                     pqrst_wave_statistics['qsi_pearson_coeff'], pqrst_wave_statistics['qsi_pearson_p_value'] = sp.stats.pearsonr(qsi[0:-2], qsi[1:-1])
                # else:
...
#             pqrst_wave_statistics['qrs_energy_std'] = np.std(qrs_eng, ddof=1)
            # if self.templates_good.shape[1] > 1:
#                 pqrst_wave_statistics['qrs_energy_approximate_entropy'] = self.safe_check(pyeeg.ap_entropy(qrs_eng, M=2, R=0.1*np.std(qrs_eng)))
                # try:
...
#                     pqrst_wave_statistics['qrs_energy_pearson_coeff'], pqrst_wave_statistics['qrs_energy_pearson_p_value'] = sp.stats.pearsonr(qrs_eng[0:-2], qrs_eng[1:-1])
                # else:
...
#         r_peak_polarity_statistics['positive_r_peaks'] = np.sum(r_peak_positive) / self.templates_good.shape[1]
#         r_peak_polarity_statistics['negative_r_peaks'] = np.sum(r_peak_negative) / self.templates_good.shape[1]
...
#             r_peak_amplitude_statistics['rpeak_kurtosis'] = sp.stats.kurtosis(rpeak_amplitudes)
            ...
            r_peak_amplitude_statistics['rpeak_approximate_entropy'] = self.safe_check(pyeeg.ap_entropy(rpeak_amplitudes, M=2, R=0.1*np.std(rpeak_amplitudes))) # keep
#             r_peak_amplitude_statistics['rpeak_sample_entropy'] = self.safe_check(ent.sample_entropy(rpeak_amplitudes, sample_length=2, tolerance=0.1*np.std(rpeak_amplitudes))[0])
#             r_peak_amplitude_statistics['rpeak_multiscale_entropy'] = self.safe_check(ent.multiscale_entropy(rpeak_amplitudes, sample_length=2, tolerance=0.1*np.std(rpeak_amplitudes))[0])
#             r_peak_amplitude_statistics['rpeak_permutation_entropy'] = self.safe_check(ent.permutation_entropy(rpeak_amplitudes, order=2, delay=1))
#             r_peak_amplitude_statistics['rpeak_multiscale_permutation_entropy'] = self.safe_check(ent.multiscale_permutation_entropy(rpeak_amplitudes, m=2, delay=1, scale=1)[0])
...
#             r_peak_amplitude_statistics['rpeak_kurtosis'] = np.nan
            r_peak_amplitude_statistics['rpeak_approximate_entropy'] = np.nan # keep
...
#             template_correlation_statistics['template_corr_coeff_std'] = np.nan
    ...
    def calculate_p_wave_correlation_statistics(self):
        ...
        if self.templates_good.shape[1] > 1:
            ...
            # Calculate correlation matrix statistics
            p_wave_correlation_statistics['p_wave_corr_coeff_median'] = np.median(upper_triangle) # keep
#             p_wave_correlation_statistics['p_wave_corr_coeff_std'] = np.std(upper_triangle, ddof=1)

        else:
            p_wave_correlation_statistics['p_wave_corr_coeff_median'] = np.nan # keep
...
#             bad_template_correlation_statistics['bad_template_corr_coeff_std'] = 0
    ...
    def stationary_wavelet_transform(self, waveform, wavelet, level):
        ...
        for lev in range(len(swt)):
            ...
            # Approximation
            swt[lev]['a'] = swt[lev]['a'][pad_before:len(waveform_padded)-pad_after] # keep

            # Detail
            swt[lev]['d'] = swt[lev]['d'][pad_before:len(waveform_padded)-pad_after] # keep
...
#             stationary_wavelet_transform_features['template_swt_d_' + str(level+1) + '_low_power_ratio'] = max_power_low / mean_power_low
#             stationary_wavelet_transform_features['template_swt_d_' + str(level+1) + '_med_power_ratio'] = max_power_med / mean_power_med
#             stationary_wavelet_transform_features['template_swt_d_' + str(level+1) + '_high_power_ratio'] = max_power_high / mean_power_high
            ...
            # Calculate max/mean power ratio
#             stationary_wavelet_transform_features['template_swt_a_' + str(level+1) + '_low_power_ratio'] = max_power_low / mean_power_low
#             stationary_wavelet_transform_features['template_swt_a_' + str(level+1) + '_med_power_ratio'] = max_power_med / mean_power_med
#             stationary_wavelet_transform_features['template_swt_a_' + str(level+1) + '_high_power_ratio'] = max_power_high / mean_power_high
