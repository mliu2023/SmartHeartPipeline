...
from biosppy.signals.ecg import ecg, hamilton_segmenter
...
def extract_rpeaks(ecg_signal):
    rpeaks = []
    for i in range(ecg_signal.shape[0]):
        rpeak_list = hamilton_segmenter(signal=ecg_signal[i], sampling_rate=500)
        rpeak_list = rpeak_list[0]
        rpeaks.append(rpeak_list.tolist())
    return rpeaks
...
def extract_features(ecg_signal):
    ...
    #### heart rate features ####
    features['hr_min'] = np.amin(heart_rate)
    features['hr_max'] = np.amax(heart_rate)
    features['hr_mean'] = np.mean(heart_rate)
    features['hr_median'] = np.median(heart_rate)
    features['hr_std'] = np.std(heart_rate)
    features['hr_activity'] = np.mean(heart_rate**2)

    #### RR interval features ####
    features['rr_min'] = np.amin(rr_intervals)
    features['rr_max'] = np.amax(rr_intervals)
    features['rr_mean'] = np.mean(rr_intervals)
    features['rr_median'] = np.median(rr_intervals)
    features['rr_std'] = np.std(rr_intervals)
    features['rmssd'] = np.sqrt(np.mean(rr_intervals**2))
    ...
    features['rr_cluster_slope'] = rr_cluster_slope
    features['rr_cluster_1'] = rr_cluster_1
    features['rr_cluster_2'] = rr_cluster_2
    features['rr_cluster_3'] = rr_cluster_3
    ...
    features['rr_fisher'] = rr_fisher

    #### diff RR interval features ####
    features['diff_rr_min'] = np.amin(diff_rr_intervals)
    features['diff_rr_max'] = np.amax(diff_rr_intervals)
    features['diff_rr_mean'] = np.mean(diff_rr_intervals)
    features['diff_rr_median'] = np.median(diff_rr_intervals)
    features['diff_rr_std'] = np.std(diff_rr_intervals)

    # pNN features
    features['pnn30'] = sum(abs(diff_rr_intervals) > 0.03) / len(diff_rr_intervals) * 100
    features['pnn40'] = sum(abs(diff_rr_intervals) > 0.04) / len(diff_rr_intervals) * 100
    features['pnn50'] = sum(abs(diff_rr_intervals) > 0.05) / len(diff_rr_intervals) * 100
    features['pnn60'] = sum(abs(diff_rr_intervals) > 0.06) / len(diff_rr_intervals) * 100
    features['pnn70'] = sum(abs(diff_rr_intervals) > 0.07) / len(diff_rr_intervals) * 100
    features['pnn80'] = sum(abs(diff_rr_intervals) > 0.08) / len(diff_rr_intervals) * 100
    features['pnn90'] = sum(abs(diff_rr_intervals) > 0.09) / len(diff_rr_intervals) * 100

    #### amplitude features ####
    features['rpeak_min'] = np.amin(rpeak_amplitudes)
    features['rpeak_max'] = np.amax(rpeak_amplitudes)
    features['rpeak_mean'] = np.mean(rpeak_amplitudes)
    features['rpeak_median'] = np.median(rpeak_amplitudes)
    features['rpeak_std'] = np.std(rpeak_amplitudes)
    
    #### entropy features ####
    features['rpeak_ent'] = pyeeg.ap_entropy(rpeak_amplitudes, M=2, R=0.1*np.std(rpeak_amplitudes))
